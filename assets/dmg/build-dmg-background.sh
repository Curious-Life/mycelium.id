...
#
#   dmg-background.svg  ──rsvg──▶  560x380 @72dpi  ┐
#                       ──rsvg──▶ 1120x760 @144dpi ┴─tiffutil─▶ dmg-background.tiff
#
# Why a multi-rep TIFF (not a plain PNG): macOS Finder paints a DMG background at
# its *point* size, unscaled. A bare 1120x760 PNG would render at 1120x760 POINTS
# and be clipped to the window's top-left quarter. The Apple-sanctioned retina
# vehicle is a HiDPI TIFF: a 560x380 base rep + a 1120x760 @2x rep combined with
# `tiffutil -cathidpicheck`, so the window is 560x380 pt and stays crisp on retina.
#
...
echo "[dmg-bg] rendering 1x (560x380) + 2x (1120x760) from SVG…"
rsvg-convert -w 560  -h 380 dmg-background.svg -o dmg-background.png
rsvg-convert -w 1120 -h 760 dmg-background.svg -o dmg-background@2x.png
