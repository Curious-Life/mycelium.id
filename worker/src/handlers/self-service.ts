 ...
 *   - SYSTEM_KEY (operator infra) and USER_MASTER_KEY (customer data) are
 *     unrelated to this layer — the Worker is still ciphertext-only.
...
export async function handleNotifyPeer(request: Request, env: Env): Promise<Response> {
  ...
  const sendRes = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: fromAddress,
      to: job.email,
      subject: `@${cleanFrom} wants to connect on Mycelium`,
