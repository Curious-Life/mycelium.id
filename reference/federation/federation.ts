 ...
 * See docs/architecture/FEDERATION-SPEC.md for full protocol.
 */

import type { Env } from "../types/env";
import { bustDidCache } from "./did-document";
...
async function handleRotateKey(request: Request, env: Env): Promise<Response> {
  ...
  // Bust the DID document cache for the rotating instance — the cached doc
  // advertises the OLD key, so peers fetching it would still trust stale
  // crypto until TTL expires (1h). Explicit bust closes that window.
  await bustDidCache(env, auth.payload!.iss);
