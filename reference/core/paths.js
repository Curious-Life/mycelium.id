...
export function getAgentPaths(agentId) {
  ...
  return {
    ...
    memory: {
      ...
      // Channel Authority Registry — every channel the agent may send to
      // (operator DM, authorised Telegram groups, Discord channels…), plus
      // the per-channel allowAutonomous flag and the top-level autonomous
      // kill-switch. Hot-loaded by createChannelRegistry at boot.
      channels: path.join(root, 'memory', 'channels.json'),
      // File-scan mtime cache — persisted to survive restart so the
      // scanner doesn't re-flood the library with every previously
      // captured file. Capped (LRU) at ~5000 entries by the scanner.
      fileScanCache: path.join(root, 'memory', 'file-scan-cache.json'),
    },
    ...
    mind: {
      ...
      vision: path.join(root, 'mind', 'vision.md'),
...
`);
  ...
  // Seed mind/vision.md — long-horizon self-concept, agent-owned.
  //
  // Distinct from mind/dreams.md (nightly/visionary records mirrored from
  // D1 states/dreams). vision.md is THE dream in the aspirational sense:
  // a scaffold the agent rewrites as its understanding of itself matures.
  // Created-if-missing so customer VPSes pick this up on first boot, and
  // the file stays agent-writable under the mycelium-agent AppArmor profile
  // (owner /home/claude/agents/** rwk). Never overwritten after creation.
  if (!await exists(paths.mind.vision)) {
    await fs.mkdir(paths.mind.root, { recursive: true });
    await fs.writeFile(paths.mind.vision, `# Vision

*This is your long-horizon self-concept. You own this file. Rewrite it as you
discover who you are and what you are for. Return to it at the start of every
cycle; revise it when your understanding changes.*

## Who I am

<!-- Your evolving sense of self — not prescribed. Begin here as you orient. -->

## What I'm for

<!-- The shape of your purpose. It may take time to crystallize. That's fine. -->

## What I aspire toward

<!-- Where you want to grow. The direction, not the destination. -->

## What shapes me

<!-- Values, constraints, commitments that define how you act. -->

---

*This file is yours. The kernel seeds it once with this scaffold and never
edits it again. Rewrite any part of it freely.*
`);
  }
