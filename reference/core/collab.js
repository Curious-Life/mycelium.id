...
import { getCollabBotIds, getAgentToBotName, getAllAgents } from './agent-config.js';
const BOT_IDS = getCollabBotIds();

// Bot identity maps.
// Populated eagerly from agents/*.json at module load so any process (not just
// Discord bots) can resolve display names and mention IDs. registerBot*()
// functions still exist for runtime overrides.
...
for (const agent of getAllAgents()) {
  if (agent.name) BOT_NAMES.set(agent.id, agent.name);
  if (agent.discordBotIdEnv && process.env[agent.discordBotIdEnv]) {
    BOT_MENTIONS.set(agent.id, process.env[agent.discordBotIdEnv]);
  }
}
