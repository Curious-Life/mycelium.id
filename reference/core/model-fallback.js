...
/**
 * Get energy-aware model config — downshifts models when energy is low/critical.
 * @param {string} taskType - 'think', 'chat', 'spawn', 'research'
 * @param {'abundant'|'normal'|'low'|'critical'} energyLevel
 * @returns {Object} Model config with primary + fallbacks
 */
export function getEnergyAwareModelConfig(taskType, energyLevel) {
  const base = TASK_MODEL_CONFIGS[taskType] || DEFAULT_MODEL_CONFIG;
  if (!energyLevel || energyLevel === 'normal' || energyLevel === 'abundant') return base;
  if (energyLevel === 'critical') {
    // Force cheapest model
    const cheapest = base.fallbacks[base.fallbacks.length - 1] || base.primary;
    return { primary: cheapest, fallbacks: [] };
  }
  if (energyLevel === 'low') {
    // Downshift primary by one tier
    const downshifted = base.fallbacks[0] || base.primary;
    return { primary: downshifted, fallbacks: base.fallbacks.slice(1) };
  }
  return base;
}
