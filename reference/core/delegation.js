...
import { announceToCollab, getBotLabel, getBotMention } from './collab.js';
...
/**
 * Delegate to a local agent.
 *
 * Delivery: direct POST to the target's /chat endpoint with sourceAgent + priority
 * + context. /chat already handles inter-agent requests (lane queue, coalescing,
 * response storage). The target sees the task as a real message, not a hint to
 * check a queue — so it actually gets processed.
 *
 * Audit: a JSON file is also written to the target's tasks/queue/ purely as an
 * audit log so we can trace who-asked-whom. It's not load-bearing for delivery.
 */
async function delegateLocal(runtime, targetAgent, { target_agent, task, priority, context }) {
  ...
  const normalisedPriority = priority || 'normal';
  const timeoutMs = normalisedPriority === 'high' ? 10 * 60 * 1000 : 30 * 60 * 1000;
  ...
  // Audit log: write to target's queue dir so history is visible (non-load-bearing)
  ...
  try {
    ...
    console.warn(`[Delegation] Audit-log write failed (non-fatal) for ${taskId}:`, error.message);
  }

  // Track locally for timeout detection
  await trackOutboundDelegation(runtime, taskMeta);

  // Direct delivery: POST to target's /chat endpoint as an inter-agent request.
  // Async — don't wait for full Claude run; the target will store the response
  // in D1 and (where wired) post to its Discord/Telegram channel itself.
  ...
  if (getWorkerSecret()) {
    internalHeaders['Authorization'] = `Bearer ${getWorkerSecret()}`;
  } else if (process.env.AGENT_TOKEN) {
    internalHeaders['Authorization'] = `Bearer ${process.env.AGENT_TOKEN}`;
  }
  ...
  const chatBody = {
    ...
    channelId: `delegation_${taskId}`,
    taskType: normalisedPriority === 'high' ? 'chat' : 'research',
  };

  fetch(`${targetAgent.url}/chat`, {
    method: 'POST',
    headers: internalHeaders,
    body: JSON.stringify(chatBody),
    signal: AbortSignal.timeout(5_000), // just for connection — server processes async
  }).catch(err => {
    // Connection-level failure (target died between health-check and now). Log and
    // fall back to wake — at minimum the audit-log file is on disk.
    console.warn(`[Delegation] Direct /chat POST failed for ${taskId} → wake fallback:`, err.message);
    requestWakeHttp(target_agent, targetAgent.url).catch(() => {});
  });

  logEvent(runtime, 'delegation.created', {
    payload: { targetAgent: target_agent, taskId, priority: normalisedPriority },
  });

  // Announce to #agent-collab. Uses real @mention of the target so the user can
  // click through to the bot, but the target's Discord bot recognises the 📋
  // announcement prefix (coming from another bot) and skips it to avoid a loop
  // with the direct /chat delivery above.
  const priorityTag = normalisedPriority === 'high' ? ' [high]' : '';
  const fromLabel = getBotLabel(runtime.agentId);
  const toMention = getBotMention(target_agent); // real <@id> if registered, else agentId
  const header = `📋 ${fromLabel} → ${toMention}${priorityTag} \`${taskId}\``;
  // Discord hard limit is 2000 chars per message. Keep header + task well under that.
  const DISCORD_BODY_LIMIT = 1800;
  const body = task.length > DISCORD_BODY_LIMIT ? task.slice(0, DISCORD_BODY_LIMIT - 3) + '...' : task;
  announceToCollab(runtime.agentId, `${header}\n${body}`);

  return `Task ${taskId} delegated to ${target_agent}. They'll respond within ${timeoutMs / 60000} minutes.`;
}
