<script lang="ts">
	...
	interface Folder {
		...
		doc_count?: number;
		// PR 5.4-D: agent ownership marker. Set on folders auto-created
		// by an agent's first publishArtifact (e.g. 'research-agent').
		// Null = user-created folder (default).
	...
	interface AgentRegistryEntry {
		...
		name: string;
		role: string;
		// One of the symbolic names from agents/*.json (jade, aurum, ...).
		// We render it via the corresponding --color-accent-<name> CSS
		// custom property — palette-aware in light/dark mode.
		color: string;
	}
	...
	// PR 5.4-D: agent_id → palette color, populated once on mount from
	// /portal/agents/registry (lightweight — no health checks). Used to
	// paint a small colored dot next to folders that belong to a
	// specific agent. Empty until the fetch returns; folders render
	// without the dot in the meantime (graceful degrade).
	let agentColors = $state<Record<string, string>>({});
	...
	onMount(async () => {
		...
		await Promise.all([loadFolders(), loadAgentRegistry()]);
		loading = false;
	});

	async function loadAgentRegistry() {
		try {
			const res = await api('/portal/agents/registry');
			if (!res.ok) return;
			const data = await res.json();
			const map: Record<string, string> = {};
			for (const a of (data.agents || []) as AgentRegistryEntry[]) {
				if (a?.id && a?.color) map[a.id] = a.color;
			}
			agentColors = map;
		} catch {
			// Best-effort; folders render without dots if this fails.
		}
	}
...
{#snippet folderItem(folder: FolderNode, depth: number)}
	...
	{:else}
		...
		>
			...
			{#if folder.agent_id && agentColors[folder.agent_id]}
				<span
					class="agent-dot flex-shrink-0"
					style="background: var(--color-accent-{agentColors[folder.agent_id]});"
					title="Created by {folder.agent_id}"
					aria-hidden="true"
				></span>
			{/if}
...
<style>
	...
	/* Small dot next to agent-owned folder names. Color comes from
	   the agent's palette token (var(--color-accent-<color>)). */
	.agent-dot {
		width: 0.5rem;
		height: 0.5rem;
