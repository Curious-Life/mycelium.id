 ...
 *
 * Usage: MYA_WORKER_URL=... AGENT_TOKEN=... node scripts/compute-cofire.js
 *   Or:  ADMIN_SECRET=... (uses admin auth)
 */
...
const TOKEN = process.env.ADMIN_SECRET || process.env.AGENT_TOKEN_MYA || process.env.AGENT_TOKEN;

if (!WORKER_URL || !TOKEN) {
  console.error('Missing: MYA_WORKER_URL, ADMIN_SECRET/AGENT_TOKEN');
  process.exit(1);
}

const headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${TOKEN}` };
if (process.env.MYA_USER_ID) headers['X-Tenant-ID'] = process.env.MYA_USER_ID;
...
async function run() {
  // Use configured owner, fall back to auto-discover
  let USER_ID = process.env.MINDSCAPE_OWNER_ID || process.env.MYA_USER_ID;
  if (!USER_ID) {
    const users = await query(`SELECT DISTINCT user_id FROM clustering_points WHERE user_id IS NOT NULL LIMIT 1`);
    USER_ID = users[0]?.user_id || 'owner';
  }
  console.log(`[cofire] Computing co-firing for user=${USER_ID}`);
  ...
  await query(`DELETE FROM territory_cofire WHERE user_id = ?`, [USER_ID]);
  ...
  for (let i = 0; i < entries.length; i += 25) {
    const batch_entries = entries.slice(i, i + 25);
    const stmts = batch_entries.map(e => ({
      sql: `INSERT INTO territory_cofire (id, user_id, territory_a, territory_b, cofire_immediate, cofire_session, cofire_daily, cofire_weekly, last_cofire_at, last_computed)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      params: [
        `${USER_ID}:${e.a}:${e.b}`,
        USER_ID, e.a, e.b,
    ...
    try {
      ...
      console.error(`[cofire] Batch at ${i} failed:`, err.message);
      // Fallback: one by one
      for (const stmt of stmts) {
        try { await query(stmt.sql, stmt.params); inserted++; } catch { /* skip */ }
      }
    }

    if (inserted % 500 === 0 || i + 25 >= entries.length) {
