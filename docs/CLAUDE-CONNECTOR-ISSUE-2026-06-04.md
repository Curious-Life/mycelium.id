...
Three attempts, **two different Claude accounts**, identical outcome:
...
A scripted **reference OAuth 2.1 + DCR + PKCE client**, run against the same live server, completes the identical flow **and then** `POST /mcp (initialize) → 200`, returning a real MCP session:
