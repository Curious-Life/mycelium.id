...
| RT2-H2 channel write-audit | deferred | **FIXED** — `channel_write_audit` (migration 0033) + `db.harness.recordWrite/listWrites`; `run-turn.js` audits write tools hash-only, fire-and-forget; owner turns wire the sink. Gate `verify:harness-state` S9 (structural-only, no plaintext). |
