...
**Recommendation:** A+B now (zero/low build, no plaintext exposure); **D (the TLS-passthrough relay) is the chosen public-grade path** — operator decision 2026-06-04, now fully designed in [`DESIGN-relay-and-gateway-2026-06-04.md`](DESIGN-relay-and-gateway-2026-06-04.md) (key finding: it needs *no* OAuth code change — `MYCELIUM_BASE_URL` already drives discovery — so it's mostly Caddy/tunnel config + a 6-line bind fix + a CT monitor). **Avoid C as anything but a labelled demo** — it contradicts the sovereignty promise (CF sees plaintext). In all cases, fix the bind: add `MYCELIUM_HTTP_HOST` (default `127.0.0.1`; the tunnel/relay explicitly opts into `0.0.0.0`) so `--http` isn't accidentally world-open (`src/server-http.js:278`).
...
2. **Harness → Mycelium gateway:** a harness points its model base-URL at Mycelium's `POST /v1/chat/completions`, which fronts the South router (§4g cascade + audited egress) — so the harness routes **both** memory (MCP) and model (gateway) through one sovereign endpoint, no keys of its own. **Now fully designed** (operator decision 2026-06-04, promoted from deferred) in [`DESIGN-relay-and-gateway-2026-06-04.md`](DESIGN-relay-and-gateway-2026-06-04.md) Part B: v1 non-streaming (router is single-shot; SSE/tools are fast-follow), Bearer-guarded, jurisdiction-gated. This is the concrete answer to "Mycelium *is* the gateway, not LiteLLM" (§4f).
...
> **Build status (2026-06-04):** lane deconflicted — the **relay (S7)** and the **North/remote-MCP** work are built by a parallel session (PRs #45/#46 incl. the loopback bind fix `server-http.js:324`, CT-monitor, tunnel); the **import-connectors + encrypted `secrets` API** by another (#67/#69/#70). This branch owns the **outbound-LLM** lane only. **S0–S3 + the Intelligence UI + S8 (gateway) are BUILT here** (verify:providers-leak / providers / resolve / egress / **gateway** all GO; full `npm run verify` chain GO). **Fast-follows (2026-06-04): S4a + S4b + `/v1/embeddings` + the S6 recommender core are now BUILT too** — designed + tracked in [`DESIGN-ai-interface-fastfollows-2026-06-04.md`](DESIGN-ai-interface-fastfollows-2026-06-04.md) (the items below are updated to match). Credential storage chose **encrypt-in-place** (option i: `ENCRYPTED_FIELDS.ai_providers=['credentials']`) over the `secrets`-table route — self-contained, so it doesn't block on #69; can migrate to the shared `secrets` store once that merges.
>
> 🔴 **Security fix shipped alongside S8 (2026-06-04):** building the gateway's end-to-end smoke surfaced a **pre-existing auth-bypass on every Bearer-guarded surface** (`/mcp`, `/ingest/*`, and the new `/v1`). `authenticate()` called `auth.api.getMcpSession({request,headers})` **without `asResponse:false`**, which returns a truthy Response-shaped `{}` for *any* input — valid token, garbage token, even no token — so **any non-empty `Bearer` header authenticated**. Fixed in `server-http.js`: pass `asResponse:false` (→ the token row for a valid token, `null` for an unknown one, mirroring better-auth's own `withMcpAuth`) **+ a fail-closed `accessTokenExpiresAt` check** (the library lookup doesn't verify expiry). Regression-guarded by a new **garbage-Bearer→401** assertion in `verify:oauth` (the gap that let it ship: the test only ever exercised the *no-token* and *valid-token* cases).

1. **S0 — Encrypt provider credentials** (landmine #1). ✅ **BUILT** — `ai_providers:['credentials']` in `ENCRYPTED_FIELDS` (`crypto-local.js`); `verify:providers-leak` proves on-disk ciphertext + `list()` omits the key + `get()` round-trips. **Blocks everything that writes a key.**
2. **S1 — Providers backend** (4c). ✅ **BUILT** — `src/portal-providers.js` (`/portal/providers` CRUD + `setActive` + `/test` probe via `src/inference/probe.js`), `db.providers` wired in `db/index.js`, mounted in `server-rest.js`. Claude-subscription-OAuth dropped (ToS); `/auth/{claude,openai}` stubs report not-connected. `verify:providers` GO. *Next (S2):* wire `src/inference/router.js` to read `db.providers.getActive()` instead of env-only.
3. **S2 — Wire router to creds** (4a). ✅ **BUILT** — `src/inference/resolve.js` `resolveInferenceConfig(db,userId)` maps the active `ai_providers` row → router opts (anthropic/openai), **authoritative over env** (returns `''` for the non-chosen vendor so a stray env key can't override); `custom`/`base_url` deferred to S3; none → `{}` (env→local fallback). Live caller `pipeline/describe-chronicles.js:166` resolves DB-first. `verify:resolve` GO; inference/chronicles no-regression GO. *Next (S3):* OpenAI-compatible `base_url` adapter in `cloud.js` + the `jurisdiction` tag + `recordEgress` at the seam (wires the `custom`/EU-sovereign tier).
4. **S3 — Widen providers + egress audit** (4b,4e). **S3a ✅ BUILT** — OpenAI-compatible `base_url` adapter in `cloud.js` (`openaiCompatibleInfer` + `resolveChatUrl`; key-optional for local servers), `baseUrl`/`jurisdiction` threaded through `router.js`, `resolve.js` maps any `base_url` provider + tags `jurisdiction` via new `src/inference/presets.js` (Regolo/Scaleway→`eu-zdr`, localhost→`local`, unknown→`us-standard` fail-safe); curated presets (OpenAI/OpenRouter/Together/Groq/**Regolo+Scaleway**/Ollama/LM Studio). `verify:resolve` extended GO (custom→base_url+jurisdiction; `cloudInfer` routes to the base_url; keyless-local no-auth); inference/chronicles no-regression GO. **S3b ✅ BUILT** — the egress boundary at the router seam: `infer({…, sensitive})` **hard-blocks** sensitive content from a `us-*` provider (falls back to on-box local; eu-zdr/local unaffected — §4g), and `onEgress` audits **every** cloud call (allowed/denied) with provider + jurisdiction + **sha256 hash + length only, never the prompt** (§4e). New `src/inference/egress.js` `createEgressAuditSink(db,userId)` routes it through the **general `db.audit` table** (decision: the `egress_audit` table is channel-shaped — telegram/discord, CHECK-constrained — a poor fit). Live caller `describe-chronicles` wired. `verify:egress` GO (7 checks: sensitive→denied→local, non-sensitive→allowed, eu-sovereign→allowed, hash-only, sink). inference/resolve/chronicles no-regression GO. **Also: the "Intelligence" Settings UI** (a separate brick) — `GET /portal/providers/presets` + `IntelligenceSection.svelte` (grouped EU-sovereign/Local/US with jurisdiction badges, connect/use/test/remove); portal build GO.
5. **S4 — North ergonomics** (3a fix, 3b, 3c). `MYCELIUM_HTTP_HOST`; opt-in static bearer; server `instructions`. **§3b static bearer ✅ BUILT** (shipped with S8) — `src/gateway/static-bearer.js` `matchStaticBearer()` (fail-closed: only when `MYCELIUM_MCP_BEARER` set, **≥24-char floor**, **constant-time** compare, never logged), wired into `authenticate()` so it covers `/mcp` AND `/v1`. **`MYCELIUM_HTTP_HOST` ✅ BUILT** (override-parity + a non-loopback warning — `:4711` was ALREADY hard-loopback, so this is the missing knob, not a fix) **+ server `instructions` ✅ BUILT** (`MYCELIUM_INSTRUCTIONS` on the MCP `Server`, `mcp.js`; SDK 1.29 emits it in `initialize`; points the client at `getContext`). Both in fast-follows Slice A.
6. **S5 — Onboarding + docs** (3d, 4c UI). "Connect Mycelium to your AI" panel + `docs/CONNECT-YOUR-AI.md` (stdio + remote per client). *Smoke:* follow the doc to attach Mycelium to a second MCP client end-to-end. **✅ BUILT (2026-06-04)** — **`docs/CONNECT-YOUR-AI.md`** (run/keys · stdio MCP for Claude Desktop/Code · HTTP MCP + the model gateway on :4711 · per-harness base-URL snippets for opencode/Codex/Goose/Cline/Continue · the static-bearer how-to · `/v1/embeddings` · the remote/relay path with an honest "relay not yet live end-to-end" note · security defaults) + a **"Connect your AI"** Settings panel (`portal-app/.../settings/ConnectYourAISection.svelte`, mounted in `SettingsView.svelte` after Remote Access): copy-paste local endpoints (MCP `:4711/mcp`, gateway `:4711/v1`, model `mycelium-auto`, embed model) + the public URLs when a handle is claimed (via `/api/v1/remote/status`) + the bearer snippet. Portal build GO; portal regressions GO.
7. **S6 — Local tier + hardware-aware recommender** (4h). `detectHardware` (Tauri Rust + Node fallback) + dated `models-catalog.json` + `recommendModels` + one-click `ollama pull`→provider row. *Smoke:* on this machine it detects RAM and recommends a model that fits the headroom budget, then registers it as a `local` provider. (After S1; can land with S3.) **Core ✅ BUILT (2026-06-04)** — `src/hardware/{fit,catalog,detect,recommend,ollama}.js`: computed-fit ranking (borrowed from odysseus `hwfit`, MIT-attributed) over a curated catalog; `detectHardware` covers NVIDIA / Apple-unified / AMD / CPU (fail-soft, `execFile` no-shell); HTTP-only ollama `pull`/`tags` with validated names. `verify:hardware` GO (17 checks). **Routes + UI ✅ BUILT** — `src/portal-hardware.js` (detect / recommend / catalog-constrained streaming pull) mounted in `server-rest.js`, + the "Recommended for your hardware" panel in `IntelligenceSection.svelte` (detect → ranked fit badges → one-click pull w/ progress → auto-register the local provider); `verify:hardware-routes` GO (5) + portal build GO. **S6 is end-to-end.**
8. **S8 — OpenAI-compatible gateway** (Part B of `DESIGN-relay-and-gateway`). ✅ **BUILT** — `src/gateway/openai-compat.js`: `POST /v1/chat/completions` + `GET /v1/models` on the Bearer-guarded `:4711` app (`server-http.js`), composing the S2/S3 seams (`resolveInferenceConfig` → `createInferenceRouter({…,onEgress})` → `infer`). `messages[]`→role-tagged prompt (system→preamble); active provider decides the route; **non-streaming v1** (`stream:true` → single terminal SSE chunk); `tools` dropped; **`X-Mycelium-Sensitive: true`** opts a request into the §4g hard-block (default false). Egress audited hash-only via the same `db.audit` sink. Auth: OAuth Bearer **or** the §3b static bearer. `verify:gateway` GO (14 checks: prompt mapping · OpenAI envelope · Bearer-guard 401 · hash-only audit · sensitive→local · stream shim · static-bearer fail-closed) + a real-server end-to-end smoke GO. **`/v1/embeddings` ✅ BUILT (2026-06-04)** — a LOCAL-ONLY adapter over the Nomic embed-service (`src/gateway/embeddings.js`; never a cloud embeddings provider — §7); `input` string|array, `encoding_format` float|base64, `X-Mycelium-Embed-Task` override; `/v1/models` advertises `nomic-embed-text-v1.5`; `verify:embeddings-gateway` GO (6 checks). **true token-streaming ✅ BUILT** (fast-follows Slice B — `inferStream` on the router + cloud SSE (OpenAI/Anthropic) / local NDJSON; the gateway pipes real deltas; same §4g + audit semantics; `verify:gateway-stream` GO). **tool pass-through ✅ BUILT** (Slice C2 — transparent OpenAI-compatible proxy so `tool_calls` round-trip; sensitive+US refused; `verify:gateway-tools` GO) **+ §4g multi-provider cascade ✅ BUILT** (Slice C1 — eu→frontier→local with a local floor, sensitive drops US; `verify:cascade` GO) **with an in-app toggle** — Settings → Intelligence "Smart routing" (`GET/PUT /portal/providers/routing` persisted in `users.settings`; the gateway reads it **DB-first** via `isCascadeEnabled`, `MYCELIUM_INFER_CASCADE` is the env fallback, default off; `db.users` wired into `db/index.js`; `verify:gateway` G11 + `verify:providers` P12 GO). **All S8 fast-follows complete** (named deferrals: streaming-for-cascade, Anthropic/local tool translation).

S0→S3 + S8 = South (Mycelium calls models); S4→S5 = North (models call Mycelium); S6 = the local-tier UX. Ship South first.
...
| `verify:gateway` (new) | `messages[]`→prompt mapping; OpenAI ChatCompletion envelope; Bearer-guard 401; **one hash-only egress row per call**; `X-Mycelium-Sensitive`→local hard-block; `stream:true` terminal chunk; `matchStaticBearer` accept + fail-closed |
| `verify:mcp` (extend) | `initialize` returns the `instructions` preamble; static-bearer path accepts/refuses correctly |
| `verify:oauth` (extended) | OAuth path GO **+ a garbage non-empty Bearer is rejected (401)** — the regression guard for the `getMcpSession` `asResponse:false` bypass fix |
