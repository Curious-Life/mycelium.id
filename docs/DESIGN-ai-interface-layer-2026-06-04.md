...
5. **S4 — North ergonomics** (3a fix, 3b, 3c). `MYCELIUM_HTTP_HOST`; opt-in static bearer; server `instructions`. **§3b static bearer ✅ BUILT** (shipped with S8) — `src/gateway/static-bearer.js` `matchStaticBearer()` (fail-closed: only when `MYCELIUM_MCP_BEARER` set, **≥24-char floor**, **constant-time** compare, never logged), wired into `authenticate()` so it covers `/mcp` AND `/v1`. *Remaining:* `MYCELIUM_HTTP_HOST` default-loopback + server `instructions` preamble.
