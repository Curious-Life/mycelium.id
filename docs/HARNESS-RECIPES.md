...
> **Local-only today.** Everything below runs on **this machine** (loopback). Remote (`https://<handle>.mycelium.id/…`) needs the relay, which is **not live end-to-end yet** — see CONNECT-YOUR-AI.md §6. Until then, connect locally.
...
> ```
> Use that exact value as the **Bearer / API key** everywhere below. Browser-based MCP clients can use **OAuth 2.1** instead (no token needed). Full detail: CONNECT-YOUR-AI.md §4.
...
```
Restart the client; Mycelium's tools appear and `getContext` orients it automatically. (HTTP MCP at `:4711/mcp` with a bearer also works — see CONNECT-YOUR-AI.md §2.)
...
**Model (gateway)** — add an OpenAI-compatible provider with `baseURL: http://127.0.0.1:4711/v1`, the bearer as the key, model **`mycelium-auto`**. See CONNECT-YOUR-AI.md §3.
...
Goose, Cline, Continue, Codex, OpenHands, Cursor, … all fit this shape — see CONNECT-YOUR-AI.md §3. Set the active provider + jurisdiction in **Settings → Intelligence**; mark a request sensitive with `X-Mycelium-Sensitive: true` to hard-block it from US providers.
...
*Config key names for third-party harnesses were verified against each project's current docs on 2026-06-06; if a harness has since changed its schema, its own MCP docs are authoritative. Design rationale: DESIGN-harness-connect-2026-06-06.md.*
