...
- **Steps 7b/8/9 — pending, need the live environment.** `personaClaims` MCP tool + searchMindscape routing change the **tool-list/discovery surface** → require the real-WebKit-browser MCP check per CLAUDE.md (curl gives a false green). Portal (`/portal/claims/*` + `ClaimsView.svelte`, reuse `TimeSeries.svelte`) needs a browser live-render. Full Tier-3 end-to-end (real Ollama writing claims) needs a pulled local model. Living-docs sweep at the end.
