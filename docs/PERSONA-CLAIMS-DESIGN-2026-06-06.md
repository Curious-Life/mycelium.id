...
- **Steps 7–9 — pending** (getContext/searchMindscape grafts + `personaClaims` MCP tool; `/portal/claims/*` + `ClaimsView.svelte`; living-docs). Grafts touch the `/mcp` surface → require the real-WebKit-browser check per CLAUDE.md before "done".
