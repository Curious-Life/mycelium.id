...
### S1 — Honesty enforcement is advisory, not wired *(HIGH, systemic)* — ✅ RESOLVED 2026-06-19
`validatePresentation` / `assertNotSurfacedUnlessValidated` (`src/metrics/cvp.js:223,253`) had **zero callers** across `src/` and `packages/`. The tool formatters (`src/tools/metrics.js:133-228`) and REST routes (`src/portal-measurement.js:75-122`) emitted raw decrypted numbers with prose-only hedging. **Live-confirmed:** `cognitiveState` returned `gamma 0.0448`, `autocorr_lag1 0.341`, `H0 entropy 3.020`, `velocity z 7.63σ` — each *with* an honest hedge bundled in, but nothing forced the agent to honor it, and the gate built to stop this never fired. Safety rested on "no reader exists" for the Tier-1 affect/insight anchors, not on the chokepoint.
→ **Fix (as-built):** the gate is now a real chokepoint.
  1. **Single source of truth** — `src/metrics/surface-gate.js` maps every metric *column* (harmonic + anchor) → its presentation family and exposes `assertColumnSurfaceable` (throws `CVP_NOT_VALIDATED`/`CVP_UNKNOWN_COLUMN`, fail-closed) + `gateAnchorValue` (replaces a non-`pass` Tier-1 number with its `refusal_mode` copy, never the raw value).
  2. **Live boundary wiring** — `src/db/metrics.js` invokes `checkSurfaceable` on every column in `getCurrentWindow`/`getSeries` (mirrors `checkMetric`); the harmonic families pass (contracted, non-Tier-1), but a Tier-1 column on this path now throws before any number reaches a formatter.
  3. **Gated anchor reader** — `src/db/anchor.js` (`db.anchor`) is the ONLY sanctioned reader of `cognitive_metrics_anchor`; every scalar runs through `gateAnchorValue`, so all four pending anchor metrics return refusal copy, never a number. Wired into `src/db/index.js` as a deliberate fail-closed reader.
  4. **Guard test** — `scripts/verify-cvp.mjs` (§4 + 3d) asserts: the gate throws for all 4 anchor columns + fails closed on unknown columns; `gateAnchorValue` substitutes refusal copy; the surface-gate map == `db.metrics.METRIC_COLUMNS` (no drift); `db.metrics` invokes the gate; **no file under `src/`/`packages/` queries `cognitive_metrics_anchor` except `src/db/anchor.js`** (the no-ungated-reader invariant); and the live `db.anchor` returns refusal copy + zero raw numbers for the rows `compute-anchors.py` writes.
  5. **I7 freshness (§6)** — `src/metrics/freshness.js` is now the shared owner of the family→budget map + probe (consumed by both the REST bridge and the agent surface); `src/tools/metrics.js` prepends a family-level staleness hedge (`freshnessHedge`) to `cognitiveState`/`cognitiveHistory` metric output, distinct from per-row `low_confidence`.
...
1. **Phantom phase enum `indeterminate`** *(MED, live-caught)* — ✅ **FIXED — PR #286 (`fix/fisher-phantom-phase-milestones`).** Header showed `Phase: cycling` at all 3 levels, but milestones said "moved from cycling into **indeterminate**". `indeterminate` is in *neither* the spec enum (`cycling|exploring|consolidating|breakthrough`) nor the as-built code enum (`stable|cycling|exploring|transforming`). **Root cause:** `compute-fisher.py` wrote the sentinel *string* `'indeterminate'` into `phase_recent` for insufficient-window / low-confidence rows; `detect_milestones` (via `COALESCE(phase_recent, phase)`) then fired `cycling → indeterminate`. **Fix:** store `NULL` (mirrors `R_recent=None`) so the enum stays exactly the 4 canonical values; add a `REAL_PHASES` gate so `phase_shift`/`sustained_cycling` fire only between two real phases; `detect_milestones` reads `phase_recent` authoritatively.
2. **Duplicate / contradictory milestones** *(MED, live-caught)* — ✅ **FIXED — PR #286.** "5 active milestones" contained 3 identical `phase_shift cycling→indeterminate` and 2 identical `sustained_cycling`, all dated 2026-06-18, and the phase_shift directly contradicted the current `cycling` phase. **Root cause:** the `fisher_milestones` UNIQUE key includes `clustering_run_id`, so every clustering run re-inserts the same logical alert. **Fix:** `db.fisher.getActiveMilestones` dedups across run-id (newest kept; benefits portal + MCP); the `fisher-tools` formatter drops phase_shifts that reference a non-real phase or contradict the current realm phase. New regression gate `verify:milestone-consistency` (7/7) reproduces the exact live shape and proves both compute + read paths.
...
1. ~~**Wire the gate (S1).**~~ ✅ DONE 2026-06-19 — `assertNotSurfacedUnlessValidated` is now a real chokepoint at the db/tool boundary (`src/metrics/surface-gate.js` + `src/db/anchor.js` + `src/db/metrics.js`), with a guard test (`verify:cvp` §4/3d) enforcing the no-ungated-reader invariant, plus I7 freshness hedging in agent output. See S1 above.
2. ✅ **Fix the live milestone bug — DONE (PR #286).** Phantom `indeterminate` + duplicate/contradictory milestones eliminated at both the compute (`compute-fisher.py`) and read (`db/fisher.js` + `tools/fisher-tools.js`) layers; regression-gated by `verify:milestone-consistency`. Read-path fixes clean up already-stored rows on read; the user-visible change lands on the next app rebuild.
