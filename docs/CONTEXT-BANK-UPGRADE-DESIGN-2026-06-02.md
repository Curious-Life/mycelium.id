...
- **Phase 2 BUILT + verified (2026-06-02 PM):** `facts` table (`0005`) + `db.facts` + **`remember`** verb (33→34); `forget`/`mark` extended to `type:'fact'` (no new tools — the lean payoff); `getContext` **FACTS** section; `searchMindscape` **`relatedTo`** proactive mode + **`scope:'facts'`** listing; `hydrateMessages` now filters `forgotten_at IS NULL` unconditionally (defense-in-depth) + `sensitive=0` in proactive mode. `verify:facts` GO (17/17) + `verify:related` GO (7/7); **full `npm run verify` → 34× GO, EXIT 0**. Commits: `2789f72` (Stage A db), `e8d1d83` (Stage B tools+gates). **Three evidence-based deviations from the draft schema/plan** (all verified against live code): (1) `facts.value` is **NULLABLE** not `NOT NULL` — soft-redact must null it; (2) **`sensitive` facts are excluded from `getContext`** (it is the always-on proactive preamble) and surface only via explicit `scope:'facts'` — sharpens §3.6; (3) the **pinned search-ranking boost is DEFERRED** (not folded into Phase 2 as the open decision suggested): `pinned` isn't carried in the in-RAM index, so a ranking boost needs threading loader→`backend.add`→scorer (well beyond ~30 LOC, own gate) and `getContext` already honors pinned. Upsert safety: `ON CONFLICT … DO UPDATE SET value = excluded.value` (never a fresh `?` — `autoEncryptParams` would write plaintext). Next: Phase 3 (entities).
- **Phase 3 BUILT + verified (2026-06-02 PM):** operator confirmed **NLP-promote + curate** (§11.3). `entities` + `entity_links` (`0006`) + `db.entities`; `remember` gains `kind:'entity'`; **`link`** verb lands (34→35); `forget`/`mark` gain `type:'entity'`; `getContext` **PEOPLE & PROJECTS** (pinned-only); `searchMindscape` **`scope:'entities'`** (narrow matches show the dossier); **`promoteFromMessages`** aggregates enrichment-written `messages.entities` proper-nouns/@mentions, threshold-gated, `source='nlp'`, never clobbering a user entity. `verify:entities` GO (19/19); **full `npm run verify` → 35× GO, EXIT 0**. Commits: `4aa5f4c` (db), `13c96ce` (tools+gate). **Two more schema deviations — forced by encryption, verified vs live code:** (1) **no `UNIQUE(user_id,type,name)`** — `name` is encrypted with a random IV (non-deterministic), so a UNIQUE/`ON CONFLICT(name)` can never match; **dedup is app-layer** (scan the type, match the decrypted name case-insensitively, upsert by id). (2) `name`/`summary`/`aliases` **NULLABLE** (redact nulls them). Product call surfaced by `verify:entities`: a dedup match **preserves the canonical display-name casing** (a case variant or NLP proper-noun must not downcase a curated name). Next: Phase 4 (cold-start gating).
- **Phase 4 BUILT + verified (2026-06-02 PM):** cold-start gating per §11.4 (present-but-"not ready"). The **11 Tier-2 readers** (`getCurrentPhase`/`getTrajectoryHistory`/`getActiveMilestones`/`getTopMovers`/`getHarmonicState`/`getMetricSeries` + topology `exploreTerritory`/`mindscapeStructure`/`listTerritories`/`territoryDetail`/`timeView`) return a uniform actionable message until clustering has run; Tier-1 is never gated. Readiness probe = `db.mindscape.getNoiseStats().total > 0` (`makeTopologyReadiness`, `src/mcp.js`); **caches true once ready, re-queries while not-ready → flips mid-session on import+cluster with no restart**, fail-closed on probe error. Gate injected at the `collectTools(domains, gate)` chokepoint (wraps only `TIER2_TOOLS`); `boot()` threads it. No schema, **no new tools (still 35)**. `verify:gating` GO (8/8 incl. the mid-session flip); **full `npm run verify` → 36× GO, EXIT 0**. Commit: `1022a92`. **Caller-audit catch (`/pre-deletion-caller-audit` instinct):** `verify:metrics` M4/M5 assert the metric-domain refusal copy on an empty vault — gating would have replaced it with the not-ready message, so `verify:metrics` now seeds one `clustering_points` row (readiness=true) and tests the refusal copy on the correct "ready vault, empty metric window" path; `verify:topology` T3 was unaffected (it only checks non-crash text). Next: Phase 5 (consolidate 11 readers → 3 — the only breaking change, behind `/pre-deletion-caller-audit`).
- **Phase 5 BUILT + verified (2026-06-02 PM) — UPGRADE COMPLETE.** The 11 cluster/Fisher/metric/topology readers consolidated into **3** (`cognitiveState` · `cognitiveHistory` · `mindscape({view})`) via `src/tools/cognition.js`, which **reuses the existing handler logic verbatim** (same `db.fisher/metrics/topology` methods + formatters) → capability preserved by construction (pure tool-surface reshape, §3.8). `fisher-tools.js`/`metrics.js`/`topology-tools.js` are now internal; `TIER2_TOOLS` updated to the 3 new names (the Phase-4→5 coupling). **Net surface 35 → 27** (−11 +3). Ran behind a full `/pre-deletion-caller-audit`: REST `/api/v1/:toolName` is generic (auto-follows the handler map), `portal-app` calls none of the 11, the `db.*` methods stay; `verify:metrics`/`verify:topology` retired and replaced by `verify:cognition` (7) + `verify:mindscape` (8) parity gates. **The full-chain run caught a missed caller** — `verify:portal` P3 asserted `tools.length >= 30` (now 27) — fixed to `>= 25` (the audit's "grep one more time" lesson, surfaced by the gate). **Full `npm run verify` → 36× GO, EXIT 0.** Commits: `f0c673a` (consolidation), `73e448a` (gates + caller migrations). **All 5 phases shipped.**
...
3. **Entities:** ✅ **promote NLP + curate** (operator-confirmed at Phase 3, 2026-06-02). `remember`/`link` for user curation; `promoteFromMessages` lifts enrichment-extracted proper-nouns (threshold-gated, `source='nlp'`, no-clobber).
...
7. **Existing surface:** ✅ **slimmed** (Phase 5, built) — 11 cognitive/topology readers → 3 (`cognitiveState`/`cognitiveHistory`/`mindscape`). Surface 35 → 27.
