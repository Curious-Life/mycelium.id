...
```
importObsidianVault(db, { userId, folderPath, files?, vaultName?, enqueueEnrichment }) -> { scanned, documentsUpserted, memoriesCreated, memoriesDeduped, folders, skipped, errors }
```
- Two input modes: `folderPath` (Node walks the dir, native path mode; vault = basename) **or** `files: [{relPath, content, mtime}]` + `vaultName` (browser `webkitdirectory` mode — the frontend sends vault-relative relPaths + the picked dir as `vaultName`, since `webkitRelativePath` includes the picked-dir segment but the Node walk doesn't). Exactly one is required.
...
- Per note, for each `{relPath, content, mtime}`:
  - **Document:** `saveDocument({db}, { userId, source:'import-obsidian', sourceType:'import_obsidian', createdBy:'import', scope:'personal', pathArgs:{name: `${vault}/${vaultRelPath}`}, folderId, content: body, title, metadata:{ tags, vault, frontmatter, relPath }, createdAt: mtime, updatedAt: mtime })` → upsert on path (clean edit re-sync); path carries the vault name so two vaults can't collide.
  - **Folders:** the vault directory structure is mirrored as a **folder tree** — a root folder named after the vault, with a subfolder per directory, and the note's `folderId` passed to `saveDocument`. Built via `db.folders` find-or-create against an in-memory `(parent_id, name)` index loaded once from `db.folders.list` — **NOT** `db.folders.ensureSubFolder`, because `folders.name` is encrypted at rest (ENCRYPTED_FIELDS) so its `WHERE name=?` can't match (it would duplicate on every re-import). Same select-all+filter pattern as secrets. Idempotent across re-imports. Traversal segments (`..`/`.`) are rejected before any folder/document write.
