...
| 3 — claims + people | person_claims, person_claim_snapshots, people | — | ⏳ next |
| 4 — bulk content | messages + facts/entities/wealth/health/tasks/reflections/chronicles/… | — | ⏳ |
...
## Cut 4 — bulk content
messages (`content/thinking/tags/entities/entity_summary/suggested_new_tag/relations/metadata/nlp_error`) + the long tail. `messages.content` is the LARGEST backfill. Gates: `verify:leak` (already reframed in cut 3), `verify:health-encryption` (HE1 — invert), `verify:import`/`:obsidian`/`:obsidian-images`/`:full-export-import`/`:frequency` FQ0 (all assert `messages.content` envelope → invert), `verify:complexity` C3 (level_name + metrics — but those are Python metrics, cut 5). Many gates. Run all referencing each table.
