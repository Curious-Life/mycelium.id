...
- **Tool surface:** corrected — **41 tools across 14 files**, not 37. Drop `delegate_to_agent` + `getTeamStatus` (multi-agent) and collapse `spaces` (multi-user) for single-user V1 → ~36 portable tools. Tools port as `createXDomain(deps) → {tools, handlers}` factories with a local `d1Query` injected (no Worker, no copy-paste-from-Worker).
...
> ### ⚠️ CORRECTED — 41 tools (not 37), in `reference/mcp-tools/`, not "the Worker"
>
> Verified reality (`reference/mcp-tools/`, 14 files; `health.js` read directly):
...
> - **DB access is dependency-injected:** handlers call a closure-captured `db` namespace (e.g. `db.health.getRange(userId, …)`) backed by a `d1Query(sql, params)` function. **Provide a local `d1Query` over the D1 Adapter (Component 1) and the entire `reference/core/db-d1/` data layer ports unchanged** — this is the real "0 rewrites" path, cleaner than copy-pasting from a Worker.
> - **Single-user surface (~36 tools):** **drop** `delegate_to_agent` + `getTeamStatus` (`delegation.js` — multi-agent, hard blockers) and **collapse** `create_space`/`seed_space`/`list_spaces` (`spaces.js` — multi-user governance). `flagForDiscussion` (`internal.js`) ports but needs the mind-file preload contract wired into the session.
...
>
> Register the ~36 single-user tools by porting the `reference/mcp-tools/*` factories and wiring their `tools`/`handlers` into the `McpServer`. The table below is the original (under-counted) reference.
