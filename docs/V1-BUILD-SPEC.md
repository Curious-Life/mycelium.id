...
| D4 | **Master key representation** | **64-char hex strings (32 bytes each). No BIP-39 words.** | The unlock secret is a copy-pasteable hex blob — identical in form to the existing `ENCRYPTION_MASTER_KEY`, so the current tmpfs/env load path is reused verbatim. **BIP-39 is dropped from V1** (`bip39.ts` / `scoped-keys.ts`-from-mnemonic are deleted from the plan). A **Key-Check Value** (KCV: stored ciphertext of a known constant) guards against a mistyped/truncated paste, since hex carries no checksum. **Note: D6 makes this two independent hex keys** (USER_MASTER + SYSTEM_KEY), each with its own KCV. **⚠️ AMENDED — as-built (#36): a SINGLE recovery key (USER_MASTER) — see D6.** Created via the app's first-run `/setup` ceremony or `npm run set-keys` (one key shown once, save-it gate), re-viewable in Settings, and restorable on a new machine by pasting it (KCV-verified before write). A lost key remains unrecoverable by design. |
...
| D6 | **System key** | **Two independent hex keys: USER_MASTER + SYSTEM_KEY.** | Keep the two-key family from `crypto-local.js`. USER_MASTER wraps user-scope DEKs; SYSTEM_KEY wraps system-scope material; the `secrets` table stays **encrypted-at-rest** under SYSTEM_KEY. Each key is a 64-char hex blob with its **own KCV**. Two env/tmpfs slots, two unlock checks at boot — fail closed if either is missing or its KCV mismatches. **⚠️ AMENDED — as-built (#36, 2026-06-02): SINGLE recovery key.** The user saves only **USER_MASTER**; SYSTEM_KEY is **HKDF-SHA256-derived** from it (`info="mycelium:system-key:v1"`, `src/account/keystore.js`). The keys are therefore **no longer independent** (USER_MASTER compromise ⇒ SYSTEM_KEY) — accepted because SYSTEM_KEY only encrypts the normally-empty operator `secrets` table and USER_MASTER already decrypts the whole vault. Both keys are still **written to the Keychain** and both **per-key KCVs** still verify at boot, so the unlock path is unchanged. The vault also moved to a **durable per-OS data dir** (`src/paths.js`, survives app updates) and the REST server boots in **setup mode** when no keys exist (serves the `/setup` ceremony, vault routes 503-guarded, completes boot in-process). See [`docs/ACCOUNT-AND-DATA.md`](ACCOUNT-AND-DATA.md). |
...
- **Search (`tools/search.ts`):** corrected — `searchMindscape` is **not** FTS5+SQL. Port the in-RAM `mind-search` subsystem (ANN + BM25 + RRF, rehydrated at boot from decrypted content). FTS5 exists in the schema but is unused. **As-built (2026-06-17):** the index now spans messages + topology profiles + **documents** (documents BM25-only via `skipEmbed`; semantic doc ranking = deferred Phase 2). `docs/DOCUMENT-SEARCH-DESIGN-2026-06-17.md`.
