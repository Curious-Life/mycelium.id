...
- **D-1 — candidate-generation method.** 256-d matryoshka (proven 100 % recall@10 / 550 ms on synthetic; reuses existing 256-d projection) vs binary quantization (30 ms but recall unproven on synthetic). **Gate: re-run [bench2.mjs](../spike/sqlite-vec-encrypted/bench2.mjs) against a decrypted sample of the real vault's `embedding_768`.** Recommendation: **256-d as the safe default; adopt binary only if real-embedding recall@10 ≥ 95 %** (it would cut query latency ~18×). Hybrid (binary candidate → 256 mid → 768 rescore) is a later optimization, not v1.
