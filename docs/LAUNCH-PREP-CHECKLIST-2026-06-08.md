...
- ✅ **Generate + async chronicles verified end-to-end** — regenerated on the rebuilt app: foreground **26s** (was 9.5min), background narrator fired, **all 9 territories narrated** with real story arcs/archetypes. (Note: the literal `chronicle` DB column is vestigial/unused — narrative lives in `story_*`/`archetype_*`/`essence`; an early check of the wrong column falsely read "0/9".)
...
- [x] **B1 · Onboarding enrichment endpoints + dismiss (FIXED — branch `fix/async-chronicles-mindscape-polling`, PR-A).** Mounted `GET /portal/enrichment/status`, `POST /portal/enrichment/trigger`, `GET /portal/enrichment/progress/:jobId`, `POST /portal/onboarding/dismiss` in `src/portal-compat.js` as thin wrappers over the embedded/total/pending counts + a module-level drainer `nudge()` (`src/enrich/drainer.js`); also fixed `/onboarding/status` to return real `enrichedCount`/`enrichmentPending`. In-process route test: 5/5 GO.
- [x] **B1b · Post-import visibility (FIXED — PR-A).** `ImportView.svelte` now shows the "Go to Mindscape" CTA for every recognized import (was gated on an `enrichmentJobId` the backend never set) + a "View in Timeline" link for chat imports (which land as messages, not documents).
- [x] **B1d · Mindscape doesn't detect data without a reload (FIXED — branch `fix/async-chronicles-mindscape-polling`, `bf5ad4b`).** Was: `checkGenerationState()` ran only in `onMount` (MindscapeView.svelte); the tabbed workspace keeps the view alive, so post-import it never re-checked → stuck on "Welcome." Fix: poll until data is detected (Generate CTA then renders on its own) + re-check on app refocus (also reloads territories so background chronicles appear).
- [x] **B1e · Chronicle narration stalled Generate (FIXED + e2e-verified — same branch/commit).** Was: Step 3 narrated each territory via an LLM inline (700 tok, 60s timeout) → froze the progress bar for minutes. Fix: deferred narration OUT of the foreground (`run-clustering.sh` gated behind `MYCELIUM_RUN_CHRONICLES=1`) into an async background pass (`startChronicleNarrationJob` in `jobs.js`, fired after a successful Generate, 180s per-territory timeout). **e2e-verified on the rebuilt app:** Generate **26s** (was 9.5min, ~20×), background job fires, and **all 9 territories narrated** (real `story_arc`/`archetype`/`essence` via `/portal/mindscape/territories`). *Follow-up (not blocking): `narrate` routes to a configured BYOK provider first; a connected Regolo provider (model `qwen3.6-27b`) errored on `narrate` and was covered by `cloudFallbackToLocal`. Harden: cap cloud TTFB so fallback runs within budget; surface invalid-model errors in the UI. `verify:chronicles` GO.*
- [x] **B1c · Optional handle step in onboarding (FIXED — PR-B).** Added a skippable 4th step to `WelcomeModal.svelte` (live `/portal/profile/handle/check` debounce + `PUT /portal/profile`). The public `<handle>.mycelium.id` still needs the paid relay (B7/B8); this is the local profile handle.
- [x] **B2 · AI-connect leads with API key, OAuth demoted (FIXED — PR-B).** `OnboardingGuide.svelte` Step 1 now shows an API-key form (provider select → `POST /portal/providers`) instead of the stubbed OAuth buttons; `ConnectionsChecklist.svelte` defaults to the API-key tab. **Also fixed a real bug:** the API-key tab was POSTing to `/portal/settings/secret` (`CLAUDE_API_KEY`), which the inference router never reads — now both paths POST `/portal/providers` (the path that actually powers portal inference).
- [x] **B3 · TEMP debug logging removed (FIXED — PR-A).** Deleted the two `// TEMP — remove pre-merge` `console.error` lines in `src/server-http.js`. *Auth/CORS-sensitive file → re-verify with the WebKit MCP Inspector at the consolidated live test (CLAUDE.md remote gate).*
