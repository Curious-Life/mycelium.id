...
| ~~Cookie-validation mechanism unconfirmed~~ **(Spike #1 RESOLVED 2026-06-06)** | — | — | Ran with deps installed: `auth.api.getSession` **exists**; `signInEmail`→`better-auth.session_token` cookie; valid cookie→`{user}`; **no/tampered cookie→null (fail-closed)**. Both mechanisms (in-process `getSession` over shared `auth.db`, OR HTTP-forward to `:4711`) are viable — pick at 1.2 build (in-process is simpler now that `getSession` is confirmed; if used, enable WAL on `auth.db` for reader/writer concurrency). **Step 1.2 unblocked.** |
