...
- **⚠️ Encryption-coverage hardening [Wave-1 finding]:** `getEncryptedFields()` honors `MYCELIUM_ENCRYPT_ONLY` (`crypto-local.js:537`) — if set, it **silently narrows** encryption to the listed columns, leaving `entities`/`relations`/`metadata`/etc. as **plaintext** (this container ships it `=content`). V1 must log active coverage at boot and **refuse to boot** with a narrowed setting unless an explicit `I_ACCEPT_REDUCED_ENCRYPTION=1` is present — never a silent downgrade. See `docs/FOUNDATION-WAVE1-RESULT.md`.
...
- **⚠️ The orchestrator is BUILD-NEW [verified 2026-05-30].** The 43 leaf namespaces are present, but the **wiring layer (`db-d1.js`) is NOT in `reference/`**: `d1Query`/`d1QueryAdmin` (with **transparent auto-encrypt** via `autoEncryptParams`/`autoDecryptResults` — db files receive *no* `encrypt`/`decrypt`, encryption lives in the query layer), the `getDb()` assembly, and the injected helpers (`parseJson`, `randomUUID`, `now`, `parseHealthRow`, `computeHealthSummary`, `hashTokenSync`, `cofireCol`) all **must be written**. Only `agent-id-aliases.js` (`buildAgentIdFilter`/`resolveAgentIds`) ports as-is. Treat Step 3 as **build-new glue + a 43-file copy**, not a pure port — this is where the real Phase-1 work concentrates.
