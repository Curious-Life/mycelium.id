...
- **Adapter contract [Wave-1 finding]:** `autoEncryptParams` **mutates `params` in place** and **returns the rewritten SQL** (it can inject a `scope` column) — bind the mutated `params`, prepare the returned SQL, and pass `{ systemKey }` for `secrets`. See `docs/FOUNDATION-WAVE1-RESULT.md`.
