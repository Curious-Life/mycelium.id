...
| 5.3 | **Real passkey unlock** — ✅ **BUILT server+SPA (2026-06-06), ◑ device spike S2 pending.** `@better-auth/passkey` wired in `src/auth.js` (`verify:passkey` 7/7 GO; S1 bump-regression GO); SPA Face ID login + post-login enrolment in `login/+page.svelte` (portal build GO). Commits `55689ae`,`39cd50c`. Design: ◑ **DESIGNED (2026-06-06):** [`DESIGN-passkey-unlock-2026-06-06.md`](DESIGN-passkey-unlock-2026-06-06.md). Pivot: not a `better-auth` plugin (none in 1.6.13) and not a reference port (incomplete + custom sessions) → the **official separate package `@better-auth/passkey@1.6.14`** (native better-auth sessions → the gate accepts them). Gating spikes: S1 the `better-auth` patch bump must not regress MCP/OAuth; S2 WebAuthn-in-WKWebView (device) | rpID = per-box relay subdomain; auth-only (no PRF); enroll after operator-password login |
