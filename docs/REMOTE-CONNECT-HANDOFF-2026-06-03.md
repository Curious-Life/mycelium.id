...
2. **Apply the server-side fixes — `/sweep-first-design` FIRST** (the server is PROVEN functional end-to-end via `_oauth-probe.mjs`; the signing theory is DEAD — see the UPDATE + RESEARCH subsections, which supersede "Whose side?" below). In order:
   ...
   > **DESIGN LOCKED 2026-06-04 — [`docs/DESIGN-mcp-discovery-fix-2026-06-04.md`](DESIGN-mcp-discovery-fix-2026-06-04.md)** (sweep-first + verification table + threat model; **STAGED**). Sweep confirmed **we own every lever — no better-auth interception**: the `.well-known` routes are ours (`src/server-http.js:62-69`), the `/mcp` 401 `WWW-Authenticate` is our own template literal (`:133-134`), and the Caddy edge is a plain `reverse_proxy` (no path/header rewrite, `src/remote/runtime.js:70-103`). **Stage 1 (HIGH-conf):** serve a MINIMAL PRM (`{resource:${baseURL}/mcp, authorization_servers:[${baseURL}], bearer_methods_supported:["header"]}`) at BOTH root AND the suffixed path + point the 401 `WWW-Authenticate` at the suffixed url (all in `server-http.js`). **Step (b) below — drop openid — is DEMOTED to optional Stage 2:** sweep proved dropping *advertised* openid can't stop id_token (gate = requested-scope vs hardcoded `opts.scopes`, `better-auth mcp/index.mjs:511,130-136`), and the reference client already proved id_token doesn't block `/mcp`. Verify: Tier-0 `verify:mcp-discovery` (static) + Tier-1 `_oauth-probe` regression + Tier-2 operator live connect. Tasks #50–55.
