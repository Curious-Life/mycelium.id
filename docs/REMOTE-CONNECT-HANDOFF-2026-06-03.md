...
- **CONFIRMED server-side gaps (verified live, 2026-06-03):**
  1. **RFC 9728 path-suffixed PRM `/.well-known/oauth-protected-resource/mcp` → 404** (we serve ONLY the root `/.well-known/oauth-protected-resource` → 200). Claude *independently probes the path-suffixed location* (#155 trace shows `GET /.well-known/oauth-protected-resource/api/mcp`). **Real candidate.** FIX: also serve the PRM (same JSON) at the `/mcp`-suffixed well-known path.
...
2. **Apply the server-side fixes — `/sweep-first-design` FIRST** (the server is PROVEN functional end-to-end via `_oauth-probe.mjs`; the signing theory is DEAD — see the UPDATE + RESEARCH subsections, which supersede "Whose side?" below). In order:
   a. **Serve the protected-resource-metadata at the `/mcp` path-suffixed well-known** — `/.well-known/oauth-protected-resource/mcp` currently **404s** (root → 200); add a route in `src/server-http.js` returning the SAME JSON (mirror/rewrite to better-auth's root handler). **CONFIRMED gap — do this first.**
