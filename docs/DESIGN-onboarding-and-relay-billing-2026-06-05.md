...
| O6 | ✅ **DONE (2026-06-05, hermetic)** — `relay-hook.js authorizeNewProxy` denies a lapsed tenant a NEW tunnel: after the token→row lookup, `if (requireEntitlement && !registry.isEntitled(handle, now, graceMs)) → reject 'subscription required'`. **Gated on `requireEntitlement`** (= `billing.enabled`) so a free self-hosted relay (no entitlements; `isEntitled` fail-closed) is **not** gated. Read LOCALLY from the registry → a Stripe outage never drops live tunnels, only new ones pause (§6 SPOF mitigation). `Login` stays open (token-only) so the user gets a legible NewProxy rejection, not an auth failure; an established tunnel drops when its TTL slot stops refreshing. `GRACE_MS` env-tunable absorbs dunning. `verify:newproxy-auth` extended NA11–NA14 GO (off→allow · paid→allow · lapsed-past-grace→reject · lapsed-within-grace→allow). | S |
| O7 | ✅ **DONE (2026-06-05, hermetic; portal smoke pending)** — `POST /v1/billing/portal` authed by a new **action-bound `billing` ed25519 claim** (added to `CLAIM_ACTIONS`): only the vault master-key holder signs, so a leaked provision claim can't open the portal. Maps the claim's `publicKey` → `stripe_customer_id` → `customerPortalSession`; fail-closed (no billing / no nonce / bad sig / no customer → refuse). **Nonce decoupling:** billing uses its OWN in-memory nonce store via `GET /v1/billing/nonce` (NOT Turnstile-gated, since billing is signature-gated and an unentitled key just 404s) — and a billing nonce is rejected by `/v1/provision` (separate store) so it can't bypass the provision bot-gate (verified P21). `router.js GET /managed/billing-portal` signs the claim + returns the https-validated portal URL; `ManagedConnectSection` shows a **"Manage billing"** button (opens the portal) when connected. `verify:provision` P18–P21 GO; `managed-claim`/`remote-config`/`billing` regress GO. | S–M |
| O8 | 🟡 **PARTIAL** — the three-way story now lives in the Settings → **Connection** pane (managed = ManagedConnectSection · own-domain + own-relay = RemoteAccessSection), each with its own copy. **Remaining:** a dedicated first-run *wizard* step + a QR affordance for the connector URL (mobile). Deferred — net-new onboarding flow best built with eyes on the screen. | M |
| O9 | ✅ **DONE (2026-06-05, visual review pending)** — `RemoteAccessSection` now frames the own-domain (`direct`) path explicitly and adds an **"Advanced — your own relay"** disclosure: `controlPlaneUrl` + `relayAddr` inputs → `/config` (`remoteMode='own-relay'`). `/api/v1/remote/status` now returns the non-secret `controlPlaneUrl`/`relayAddr` so the fields pre-fill. `remote-config`/`loopback`/`remote-runtime` GO; build + svelte-check clean. | M |
| O10 | ✅ **DONE (2026-06-05, copy)** — lapse/release lifecycle copy on the connected managed card (next to "Manage billing"): grace period → tunnel stops at next reconnect → handle held then freed → re-subscribe keeps it. (A *live* paid_until/lapsed-state readout would need a `/v1/billing/status` endpoint — deferred.) | S |
| O11 | ✅ **DONE (2026-06-05)** — sovereignty disclosure on both managed + own-domain cards: data encrypted on the Mac, **TLS terminates on the Mac**, relay forwards **ciphertext only**, paying rents a name + passthrough pipe (not your data/keys/cert), bring-your-own = free. | S |
...
5. **O6 — relay-hook gate + grace.** ✅ `verify:newproxy-auth` (NA11–NA14) GO.
6. **O7 — billing portal + Settings "Manage billing".** ✅ DONE (P18–P21 GO).
7. **O8/O9/O10/O11 — onboarding step, own-domain/own-relay UI, lifecycle, copy.**
   ✅ O9/O10/O11 done (Settings → Connection: own-relay fields + sovereignty +
   lifecycle copy). 🟡 O8 partial — three-way story is in Settings; a first-run
   wizard step + QR is the remaining polish (build with eyes on the screen).
