...
export const IMPORT_TABLE_POLICY = Object.freeze({
  ...
  pipeline_runs: { verdict: 'deny', why: 'this box own record of which pipeline runs it executed (migration 0059). Two hazards, either sufficient: a planted `running` row is a liveness claim about a process that never existed here, and a bundle writing run history FORGES the diagnostic record the table exists to provide — the same reasoning that denies egress_audit / deployment_log (CLAUDE.md §8). Same class as the denied pipeline_state one layer up: runtime state, not vault content.' },
  pipeline_run_stages: { verdict: 'deny', why: 'the per-stage half of pipeline_runs. Denied with its parent: importing stage outcomes for runs that never happened on this box is forged provenance, and a planted `running` stage row names a stage this vault never executed.' },
