 ...
 *  The capture is `(\S.*)`, not `(.+)`: `\s+` (whitespace) and `.` overlap on
 *  spaces, so `\s+(.+)` backtracks polynomially on an attacker-controlled header
 *  of many spaces (js/polynomial-redos). Anchoring the token to a non-whitespace
 *  first char removes the overlap — linear scan, identical result after trim. */
