...
// ── one-click "save my recovery key" target (ceremony convenience) ───────────
const RECOVERY_LABEL = 'Mycelium Recovery Key';

// NOTE (D-027): a "Save to 1Password" affordance that shelled out to the `op`
// CLI was REMOVED here. 1Password's desktop-app CLI integration authorizes PER
// CALLING PROCESS via an interactive desktop approval that a GUI app cannot
// satisfy from a headless `op whoami` spawn — so every failure collapsed to a
// single "signed out" bucket and told the user to enable an integration that was
// already on (the operator's report). It is not a consumer surface. The recovery
// key is a break-glass secret; the standard, manager-agnostic way to back it up
// is copy / download / print-QR / reveal-and-transcribe (see the recovery-key
// UI), which need no OS cooperation. Do NOT reintroduce an `op`-CLI save path.
// (This is unrelated to the BOOT key-source `op read` option in
// src/crypto/key-source.js, which is a separate, deliberate deployment mode.)
...
/** Best-effort: open Keychain Access so the user SEES the saved item natively
 *  (the External-URL webview can't, so the server does it). Never throws. */
export function openInStore(target) {
  if (target !== 'keychain') return; // the only supported store target
  try { execFile('open', ['-a', 'Keychain Access'], () => {}); } catch { /* best-effort, non-fatal */ }
}
