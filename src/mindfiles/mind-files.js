 ...
 * envelope as D1 ciphertext. Format on disk:
 *   bytes 0..3   magic "MIND" (4 ASCII bytes, format v1)
 *   bytes 4..n   base64 envelope from crypto-local.encrypt()
 *
 * The 4-byte magic distinguishes an encrypted mind file from a legacy
 * plaintext .md before attempting decrypt. Files without the magic are
 * read as plaintext UTF-8 (time-boxed backwards-compat during rollout).
 *
 * Ported for V1 single-user: the legacy `@mycelium/encryption/crypto-local.js`
 * import is rewritten to the relative `../crypto/crypto-local.js`. The master
 * key flows through crypto-local's getMasterKey() (reads the user master key
 * from the env in the V1 path). Body otherwise unchanged from the reference.
 *
...
export function createMindFiles(deps) {
  ...
  // via crypto-local's inferScope. mind/ paths route to 'personal' or 'moms'.
  ...
  // Atomic encrypt-and-write: encrypt → magic-prefixed payload → tmp + fsync +
  // rename. POSIX rename is atomic same-filesystem, so a concurrent reader sees
  // either the previous content or the new — never partial.
 ...
 * updateDocument / editDocumentContent mirror writes to these files so
 * subsequent agent sessions can pre-load them without a D1 round-trip.
 */
