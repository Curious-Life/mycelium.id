...
describe('lib/agent-config.js', () => {
  ...
  before(async () => {
    mod = await import('../lib/agent-config.js');
    mod.clearCache();
  });
  ...
  it('getAgentConfig returns correct agent', () => {
    const mya = mod.getAgentConfig('personal-agent');
    assert.ok(mya);
    assert.equal(mya.name, 'Mya');
    assert.equal(mya.port, 3004);
  });
  ...
  it('getAgentNames returns { id: name } map', () => {
    ...
    assert.equal(names['personal-agent'], 'Mya');
    assert.equal(names['company-agent'], 'Com');
    assert.equal(names['research-agent'], 'Ada');
    assert.equal(names['wealth-agent'], 'Rob');
    assert.equal(names['intel-agent'], 'Apollo');
  });

  it('getAgentRegistry returns { id: { name, port, color, role } }', () => {
    const reg = mod.getAgentRegistry();
    assert.equal(typeof reg, 'object');
    const mya = reg['personal-agent'];
    assert.ok(mya);
    assert.equal(mya.name, 'Mya');
    assert.equal(mya.port, 3004);
    assert.equal(mya.color, 'azure');
    assert.equal(mya.role, 'Personal Assistant');
  });
  ...
  it('getAgentDisplayName returns name for known agents', () => {
    assert.equal(mod.getAgentDisplayName('personal-agent'), 'Mya');
    assert.equal(mod.getAgentDisplayName('research-agent'), 'Ada');
  });
  ...
  it('getCollabBotIds returns shortname → botId map', () => {
    ...
    assert.equal(typeof bots, 'object');
    // Should have entries like 'mya', 'com', 'ada', etc.
    assert.ok('mya' in bots);
    assert.ok('com' in bots);
    assert.ok('ada' in bots);
  });

  it('getAgentToBotName returns agentId → shortname map', () => {
    const map = mod.getAgentToBotName();
    assert.equal(map['personal-agent'], 'mya');
    assert.equal(map['company-agent'], 'com');
    assert.equal(map['research-agent'], 'ada');
  });
});
