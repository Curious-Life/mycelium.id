...
import { getMasterKeyFromBestSource, decrypt, isEncrypted } from './crypto-local.js';
...
/** Load master key once (lazy) — KMS (preferred) → tmpfs → env (fallback). */
async function ensureMasterKey() {
  if (masterKey) return masterKey;
  masterKey = await getMasterKeyFromBestSource();
  if (!masterKey) {
    console.warn('[secrets] No master key (no KMS, no tmpfs, no env) — secrets will not be decrypted');
  }
  return masterKey;
}
