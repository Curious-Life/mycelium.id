...
// Energy state (updated every 5 minutes, opt-in)
...
async function _updateEnergyLevel() {
  ...
  } catch { /* energy module not available — default to normal */ }
}
 ...
 * Energy-aware: skips non-essential cycles when energy is critical,
 * reduces maxTurns when energy is low.
 */
function triggerIfNotRan(dateKey, jobName, prompt, options = {}) {
  const key = `${dateKey}:${jobName}`;
  if (jobsRanToday.has(key)) return;

  // Energy-aware gating (opt-in: works without energy module)
  if (_energyLevel === 'critical' && !options.essential) {
    console.log(`[Scheduler] Skipping ${jobName} — energy critical`);
    return;
  }
  if (_energyLevel === 'low' && options.maxTurns) {
    options.maxTurns = Math.round(options.maxTurns * 0.6);
    console.log(`[Scheduler] ${jobName} — energy low, reduced maxTurns to ${options.maxTurns}`);
  }
