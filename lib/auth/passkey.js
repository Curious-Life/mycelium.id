...
export async function generateRegOptions(registrationCode) {
  ...
  // Generate random PRF salt for URK derivation
  const prfSalt = crypto.randomBytes(32);
  const prfSaltHex = prfSalt.toString('hex');
  const prfSaltBase64url = prfSalt.toString('base64url');
  ...
  // Store challenge + PRF salt keyed by registration code
  storeChallenge(`reg:${registrationCode}`, options.challenge);
  storeChallenge(`reg-prf-salt:${registrationCode}`, prfSaltHex);
 ...
 */
export async function verifyReg(registrationCode, credential, urk = null) {
  const db = getDb();
  ...
  // Retrieve PRF salt stored during generateRegOptions
  const prfSaltHex = getChallenge(`reg-prf-salt:${registrationCode}`);
  ...
  // Check PRF support from credential response
  const prfSupported = !!urk && urk.length === 64;
  ...
  return { verified: true, session, userId, urk: prfSupported ? urk : null, credentialId: cred.id, prfSupported };
}

/**
 * Generate authentication options (login).
 * Includes PRF salts for all registered credentials (for URK derivation).
 */
export async function generateAuthOptions() {
  const db = getDb();

  // Build PRF evalByCredential map for all credentials with PRF support
  let prfExtension = undefined;
  const userId = process.env.MYA_USER_ID;
  if (userId) {
    try {
      const credentials = await db.passkeys.listByUser(userId);
      const prfCredentials = credentials.filter(c => c.prf_salt);
      if (prfCredentials.length > 0) {
        const evalByCredential = {};
        for (const c of prfCredentials) {
          evalByCredential[c.credential_id] = {
            first: Buffer.from(c.prf_salt, 'hex').toString('base64url'),
          };
        }
        prfExtension = { evalByCredential };
      }
    } catch (err) {
      console.warn('[Auth] Failed to load PRF salts for auth options:', err.message);
    }
  }
  ...
  const options = await generateAuthenticationOptions({
    rpID,
    userVerification: 'required',
    extensions: prfExtension ? { prf: prfExtension } : undefined,
  });
 ...
 */
export async function verifyAuth(credential, urk = null) {
  const db = getDb();
  ...
  return { verified: true, session, userId: stored.user_id, urk: urk || null, credentialId: stored.credential_id };
}
