...
// Lazy master key — loaded once on first DB call
let _masterKey = null;
...
async function getMasterKey() {
  ...
  _masterKeyPromise = (async () => {
    // Reads from tmpfs (/run/mycelium/master.key) preferred, falls back to env.
    // Uses sodium SecureBuffer for hex/key bytes if sodium-native is available.
    _masterKey = await importMasterKeyFromTmpfs();
    if (!_masterKey) {
      console.warn('[db-d1] No master key available (tmpfs nor env) — encryption disabled');
    }
    return _masterKey;
  })();
...
function ownerHeaders() {
  ...
  const agentToken = process.env.AGENT_TOKEN;
  if (adminSecret && !isEncryptedValue(adminSecret)) {
...
/** Check if a value is encrypted ciphertext (not a usable token) */
function isEncryptedValue(val) {
  return typeof val === 'string' && val.startsWith('eyJ') && val.length > 100;
}

/** Full-scope auth for personal data (people table). Falls back to standard auth. */
function adminHeaders() {
  const adminSecret = process.env.ADMIN_SECRET;
  const tenantId = process.env.MYA_USER_ID;
  if (adminSecret && !isEncryptedValue(adminSecret)) {
    const h = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${adminSecret}` };
...
async function d1Query(sql, params = []) {
  ...
  // Auto-encrypt params on write (INSERT/UPDATE)
  // v2 envelopes include userId for per-user key isolation
  if (masterKey && params.length > 0) {
    ...
    sql = await autoEncryptParams(sql, params, scope, masterKey, userId);
  }
  ...
  // Auto-decrypt results on read (scope-enforced)
  if (masterKey && data.results?.length) {
    data.results = await autoDecryptResults(data.results, masterKey, ALLOWED_SCOPES);
  }
...
async function d1QueryAdmin(sql, params = []) {
  ...
  if (masterKey && data.results?.length) {
    data.results = await autoDecryptResults(data.results, masterKey, null);
  }
...
async function d1Batch(statements) {
  ...
  // Auto-encrypt params in each statement (v2 envelopes with userId)
  if (masterKey) {
    const scope = inferScope({ table: null, agent_id: process.env.AGENT_ID });
    const userId = process.env.MYA_USER_ID || process.env.USER_ID || null;
    for (const stmt of statements) {
      if (stmt.params?.length) {
        stmt.sql = await autoEncryptParams(stmt.sql, stmt.params, scope, masterKey, userId);
      }
    }
  }
  ...
  if (masterKey && Array.isArray(data)) {
    for (const item of data) {
      if (item.results?.length) {
        item.results = await autoDecryptResults(item.results, masterKey, ALLOWED_SCOPES);
      }
    }
  }
...
async function hybridSearch(opts) {
  ...
  const masterKey = await getMasterKey();
  if (masterKey && data.results?.length) {
    data.results = await autoDecryptResults(data.results, masterKey);
  }
  ...
  const db = {
    ...
    passkeys: {
      async listByUser(userId) {
        const result = await d1Query(
          `SELECT credential_id, public_key, counter FROM passkey_credentials WHERE user_id = ?`,
          [userId],
        );
        return result.results || [];
      },
      ...
      async create(userId, credentialId, publicKey, counter) {
        await d1Query(
          `INSERT INTO passkey_credentials (user_id, credential_id, public_key, counter) VALUES (?, ?, ?, ?)`,
