 ...
 *   POST /api/db/query   — execute SQL on D1
 *   POST /api/db/batch   — batch SQL statements
 ...
 *
 * Auth: MYA_WORKER_SECRET in Authorization header.
 */
...
/** d1Query with admin/full-scope auth for reading personal-scope encrypted data */
  ...
  return {
    ...
    messages: {
      ...
      async selectByAgent(agentId, { offset = 0, limit = 50 } = {}) {
        ...
        const result = await d1Query(
          `SELECT id, role, content, created_at, metadata FROM messages WHERE agent_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`,
