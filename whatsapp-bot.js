...
async function chatWithAgent(message, whatsappNumber) {
...
async function handleMessage(msg, sock, downloadMediaMessage) {
  ...
  console.log(`[WhatsApp] Message from ${senderNumber}: ${text.slice(0, 100)}${text.length > 100 ? '...' : ''}`);
...
async function processTextBatch(jid, text, firstKey, sock) {
  ...
  try {
    ...
    const response = await chatWithAgent(text, senderNumber);
    ...
    if (response !== null) {
      ...
      console.log('[WhatsApp] Agent chose NO_REPLY — silent processing');
    }
...
async function handleMediaMessage(msg, jid, message, sock, downloadMediaMessage) {
  ...
  try {
    ...
    const response = await chatWithAgent(messageText, senderNumber);
    ...
    if (response !== null) {
      ...
      console.log('[WhatsApp] Agent chose NO_REPLY for media — silent processing');
    }
