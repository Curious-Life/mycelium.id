...
- **Context Bank Upgrade — design spec (2026-06-02, latest).** Sweep-first-design pass closing the MCP
  ...
  Tier-2 gating, user salience, unified `ref` handle. **Phases 1–4 BUILT + verified (31→35 tools; forget 13/13, facts 17/17, related 7/7, entities 19/19, gating 8/8; full `verify` 36× GO); spec status §1, Phase 5 (breaking consolidation) pending:**
  ...
  build order = Phase 1 (forget+salience) DONE [`a200ed0`·`9cde646`·`22c1a75`] · Phase 2 (facts + `remember` + `relatedTo`/`scope:'facts'` + getContext FACTS) DONE [`2789f72`·`e8d1d83`] · Phase 3 (entities + `link` verb + NLP-promote + getContext PEOPLE + `scope:'entities'`) DONE [`4aa5f4c`·`13c96ce`] · Phase 4 (cold-start gating: 11 Tier-2 readers → uniform "not ready" until clustering, mid-session flip; no new tools) DONE [`1022a92`]; next = Phase 5 (consolidate 11 readers → 3, the ONLY breaking change, behind `/pre-deletion-caller-audit`). Rebased onto #43 (`1a8f525`). Local SQLite only — no D1/Cloudflare. Key gotchas: encrypted upserts MUST use `ON CONFLICT … DO UPDATE SET x=excluded.x` (a fresh `?` writes plaintext); can't UNIQUE an encrypted col (entity name dedup is app-layer); Phase-4 `TIER2_TOOLS` lists old names → Phase 5 rename must update it. **Pickup:** [`docs/CONTEXT-BANK-UPGRADE-HANDOFF-2026-06-02.md`](docs/CONTEXT-BANK-UPGRADE-HANDOFF-2026-06-02.md).
