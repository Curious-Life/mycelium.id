...
- **Pre-launch UX fixes — 13/17 P0+P1 DONE+LIVE; 4 remain (2026-06-16).** [`docs/PRELAUNCH-UX-HANDOFF-2026-06-16.md`](docs/PRELAUNCH-UX-HANDOFF-2026-06-16.md). Spec `~/Downloads/mycelium-coding-agent-spec-2026-06-16.md`. Local `main` = **`b22fde0`** (3 commits `5648e3f`/`26a4184`/`b22fde0`: nav→semantic anchors+#2 modal+#20 copy/chip; AI-config models endpoint+auto-fill+custom-endpoint; agent name/personality). All svelte-check 0/0 + verify:providers/chat GO + **LIVE in `/Applications/Mycelium.app`** (relaunched + verified: `/agent-identity`+`/providers/models`→200 real Ollama). 5 items already-satisfied in main (spec tested older bundle). **NOT pushed to origin** (origin/main still `337c942`; local main 3 ahead). **REMAINING (fresh session, 3-fold sweep first):** #19 public space + context areas (EXTEND Profile/handle + Sharing-contexts), #10 MCP/webhook streams, #13 galaxy + #11 streams redesign (visual, browser-verify). GOTCHA: running app = `/Applications/Mycelium.app` NOT the dev build; git pack-corruption recovered this session.
- **Nav IA refactor for launch (2026-06-08).** [`docs/NAV-IA-LOCK-2026-06-08.md`](docs/NAV-IA-LOCK-2026-06-08.md). Launch nav = **Mycelium · Library · Streams · People · Curious Life** (+ pinned Profile/Settings), down from 8 flat items. Streams = Import+Timeline merged (segmented); People = Connections/Spaces/Sharing cluster (sub-nav). Sidebar scroll-bug fixed. People/federation = **IN launch nav** (decided — supersedes CLAUDE.md "Federation: Deferred"; reconcile CLAUDE.md). Built + `verify:nav` GO 10/10 + svelte-check 0 errors. Branch `feat/nav-ia-launch` (PR carries 4 prior chat/onboarding commits too). **Next:** prune roadmap pages (agents/cycles/wealth/intel/body/vitality/activity/media/fleet/modules/chat) via [[pre-deletion-caller-audit]] once scope fully signed off.
