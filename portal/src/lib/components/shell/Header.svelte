<script lang="ts">
	...
	function handleMenuClick() {
		if (browser && window.innerWidth < 768) {
			// On mobile, could open a drawer - for now just toggle sidebar
...
<header
	class="h-14 border-b border-[var(--color-border)] flex items-center px-2 sm:px-4 gap-2 sm:gap-4 bg-[var(--color-surface)] relative z-10 overflow-hidden flex-shrink-0"
>
	<!-- Sidebar toggle (hamburger) -->
	<button
		onclick={handleMenuClick}
		class="p-2 hover:bg-[var(--color-elevated)] rounded-lg transition-colors text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] flex-shrink-0"
