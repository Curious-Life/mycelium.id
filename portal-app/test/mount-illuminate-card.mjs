...
// REAL, not stubbed — the subject binds to it (`bind:expanded`), and Svelte 5 requires the child
// to declare that prop `$bindable`, so an inert stub would throw at mount rather than degrade.
// It has no dependencies beyond svelte, so there is nothing to gain by faking it.
writeFileSync(
