...
- `/deploy-and-verify` — for any change you're about to ship. V1 self-hosted: run the `verify:*` gate(s) to `VERDICT: GO` / EXIT 0, smoke the changed surface (stdio MCP · REST+portal :8787 · publish :8788), update the living docs, emit a [✓]/[—] ledger. **Remote MCP/OAuth has now shipped (relay + browser-CORS, PR #83): any change to `src/server-http.js` auth/CORS, `src/auth.js`, discovery, or `/mcp` is NOT verified until the official MCP Inspector connects + lists tools IN A REAL BROWSER (WebKit) — `curl`/CLI skip CORS and give a false green (cost several sessions on the 2026-06-04 connector debug). See the `deploy-and-verify` skill's "Remote MCP / OAuth" recipe + `docs/REMOTE-CONNECT-HANDOFF-2026-06-03.md` (2026-06-04 section).**
