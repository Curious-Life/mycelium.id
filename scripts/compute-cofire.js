...
const TOKEN = process.env.AGENT_TOKEN || process.env.ADMIN_SECRET;
const USER_ID = process.env.USER_ID || process.env.MYA_USER_ID;

if (!WORKER_URL || !TOKEN || !USER_ID) {
  console.error('Missing: MYA_WORKER_URL, AGENT_TOKEN/ADMIN_SECRET, USER_ID/MYA_USER_ID');
  process.exit(1);
}
...
async function run() {
  console.log('[cofire] Loading clustering points...');
