...
try {
  ...
  // SQLCipher collapse (Stage B/C cut 5): the metric/distribution columns are now
  // PLAINTEXT-in-cipher — at-rest = whole-file SQLCipher (verify:at-rest), not per-field
  // envelopes. activation_vector is a readable plaintext JSON distribution now.
  const allPlain = tr && encCols.every((c) => !isEnvelope(tr[c]));
  let avPlain = false; try { avPlain = typeof JSON.parse(tr.activation_vector) === 'object'; } catch {}
  rec('FE1. trajectory sensitive columns PLAINTEXT-in-cipher (collapse cut 5; at-rest = whole-file SQLCipher, verify:at-rest)',
    !!allPlain && avPlain,
    tr ? `plain{${encCols.filter((c) => !isEnvelope(tr[c])).length}/${encCols.length}}  av_json=${avPlain}` : 'no row');
  ...
  raw.close();
  if (ms) {
    rec('FE3. milestone content PLAINTEXT-in-cipher (collapse cut 5; detail/headline/velocity_z plaintext; rule_type plaintext; verify:at-rest)',
      !isEnvelope(ms.detail) && !isEnvelope(ms.headline) && !isEnvelope(ms.rule_type)
        && (ms.velocity_z == null || !isEnvelope(ms.velocity_z)),
      `count=${msCount} rule_type=${ms.rule_type} detail_plain=${!isEnvelope(ms.detail)} headline_plain=${!isEnvelope(ms.headline)}`);
  } else {
