...
/**
 * Call Workers AI Llama 4 Scout for text generation via a Worker endpoint.
 * We'll use the tagging endpoint's underlying model.
 */
async function generateDescription(prompt) {
  // Use the Worker's /api/ai/generate endpoint if it exists,
  // otherwise fall back to embedding + manual description
  ...
  // Fallback: use the tag endpoint's pattern to call Workers AI
  // The endpoint might not exist yet — we'll add it
  throw new Error(`AI generate endpoint returned ${res.status}`);
}
...
async function getClusterSamples(clusterId, level) {
  ...
  const messages = await d1Query(`
    SELECT m.content, m.tags, m.entities
    FROM clustering_points cp
    JOIN messages m ON m.id = cp.source_id
    WHERE cp.${column} = ? AND cp.source_type = 'message'
    ORDER BY RANDOM()
    LIMIT 20
  `, [clusterId]);
  ...
  for (const realm of realms) {
    ...
    if (!force) {
      const existing = await d1Query(
        `SELECT name FROM realms WHERE realm_id = ? AND name IS NOT NULL LIMIT 1`,
        [realm.realm_id]
      );
    ...
    if (!dryRun) {
      ...
      await d1Run(`
        ...
        VALUES (?, (SELECT DISTINCT user_id FROM clustering_points WHERE realm_id = ? LIMIT 1),
                ?, ?, ?, ?, ?, 'llama-4-scout')
      ...
      `, [realm.realm_id, realm.realm_id, desc.name, desc.essence, realm.cnt,
...
async function describeTerritories() {
  ...
  for (const terr of territories) {
    if (!force) {
      const existing = await d1Query(
        `SELECT name FROM territory_profiles WHERE territory_id = ? AND name IS NOT NULL LIMIT 1`,
        [terr.territory_id]
      );
      if (existing.length > 0) {
    ...
    if (!dryRun) {
      await d1Run(`
        ...
        VALUES (?, (SELECT DISTINCT user_id FROM clustering_points WHERE territory_id = ? LIMIT 1),
                ?, ?, ?, ?, ?, ?, 'llama-4-scout')
        ON CONFLICT(territory_id, user_id) DO UPDATE SET
          ...
          raw_response = excluded.raw_response, generation_model = excluded.generation_model
      `, [terr.territory_id, terr.territory_id, desc.name, desc.essence,
